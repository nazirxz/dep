...
class ItemManagementController extends Controller
{
    ...
    {
        // Ambil semua data dari payload JSON
        $data = $request->json()->all();

        // 1. Validasi Data menggunakan Validator secara manual
        $validator = Validator::make($data, [
            'nama_barang' => 'required|string|max:255',
            'jumlah_barang' => 'required|integer|min:1',
            'satuan_barang' => 'required|string|max:50',
            'kondisi_fisik' => 'required|string|in:Baik,Rusak Ringan,Tidak Sesuai,Kadaluarsa', // <--- Diubah dari 'kondisi_barang'
            'nama_produsen' => 'nullable|string|max:255',
            // Tambahkan validasi untuk kolom lain jika diperlukan dari form verifikasi
            // Contoh: 'kategori_barang' => 'nullable|string|max:255',
            //         'metode_bayar' => 'nullable|string|max:50',
            //         'pembayaran_transaksi' => 'nullable|numeric|min:0',
            //         'nota_transaksi' => 'nullable|string|max:255',
        ]);
        ...
        try {
            ...
            $kondisiBarang = $data['kondisi_fisik']; // <--- Diubah dari 'kondisi_barang'
            $namaBarang = $data['nama_barang'];
            $jumlahBarang = $data['jumlah_barang'];
            $namaProdusen = $data['nama_produsen'] ?? null; // Gunakan null coalescing operator
            $satuanBarang = $data['satuan_barang'];

            if ($kondisiBarang === 'Baik') {
                // Jika kondisi barang 'Baik', masukkan ke tabel incoming_items
                $incomingItem = IncomingItem::create([
                    'nama_barang' => $namaBarang,
                    'kategori_barang' => $data['kategori_barang'] ?? 'Umum', // Gunakan nilai default atau ambil dari data
                    'jumlah_barang' => $jumlahBarang,
                    'tanggal_masuk_barang' => now(), // Menggunakan tanggal dan waktu saat ini
                ...
                return response()->json([
                    'success' => true,
                    'message' => 'Barang "' . $namaBarang . '" dengan kondisi baik berhasil diverifikasi dan ditambahkan ke stok.',
                    'data' => $incomingItem
                ]);

            } else {
                // Jika kondisi barang tidak 'Baik' (rusak, tidak sesuai, kadaluarsa)
                // Anda bisa menyimpan ini ke tabel terpisah untuk barang yang perlu penanganan khusus
                // Atau hanya mencatatnya di log/notifikasi tanpa menambahkannya ke stok utama.
                // Untuk contoh ini, kita hanya akan mengembalikan pesan.

                DB::rollBack(); // Rollback transaksi jika tidak dimasukkan ke incoming_items

                return response()->json([
                    'success' => true, // Tetap success karena verifikasi berhasil, hanya tidak masuk stok
                    'message' => 'Barang "' . $namaBarang . '" diverifikasi dengan kondisi "' . $kondisiBarang . '". Tidak ditambahkan ke stok utama.',
                    'data' => [
                        'nama_barang' => $namaBarang,
                        'jumlah_barang' => $jumlahBarang,
                        'kondisi_barang' => $kondisiBarang, // Menggunakan nama variabel lokal
            ...
            return response()->json([
                ...
                'errors' => [$e->getMessage()] // Mengirim pesan error untuk debugging
